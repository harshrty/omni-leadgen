...
### Serena — semantic code graph (project-scoped MCP)
Use for structural traversal: find symbols, callers, callees, imports.
Tools: `find_symbol`, `search_symbols_in_file`, `get_symbols_overview`, `list_dir`
First use each session: call `onboard_project` once, then `activate_project`.

### Issue Finder skill
Run `/issue-finder` for a full automated audit: security · silent failures · performance · data quality · dead code.
Returns a prioritised report with file:line references. Reports only — never edits.

---

## vexp - Context-Aware AI Coding <!-- vexp v2.0.24 -->
